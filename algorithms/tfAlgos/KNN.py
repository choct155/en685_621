...
class KNN:
    ...
    def get_max_k(distances: tf.data.Dataset, k: int) -> tf.data.Dataset:
        values, labels = distances.as_numpy_iterator().next()
    ...
    def get_label(max_k: tf.data.Dataset, labels: List[str]) -> str:
        ...
        for i, v in enumerate(values):
            out[0][np.argwhere(out[1] == labels[i])] += 1/v # weights decline with distance
        # returning the label with the highest aggregate, distance discounted weight
        return out[1][np.argmax(out[0])]

    def __classify_one_obs(self, obs: tf.Tensor) -> str:
    ...
    def fit(self) -> tf.data.Dataset:
       return self.test.map(lambda x, y: (x, tf.map_fn(lambda xval: self.__classify_one_obs(xval), y)))
